...
export const AccountSettings = () => {
  ...
  /* ---------------- AVATAR ---------------- */
  const [avatarPreview, setAvatarPreview] = useState<string | null | undefined>(authUser.user?.avatarUrl);
  ...
  const validate = () => {
    ...
    if (changePassword) {
      ...
      if (newPassword.trim().length < 4) return "New password must contain at least 4 characters";
    }
    return null;
  };
  ...
  return (
    /* AJUSTE: h-screen removido para permitir scroll natural do body ou do container principal */
    /* overflow-y-auto garante que se o conteúdo exceder a viewport, a barra de rolagem apareça */
    <div className="min-h-screen w-full bg-zinc-50 flex justify-center py-10 px-4 overflow-y-auto">
      <div className="w-full max-w-4xl h-fit"> {/* h-fit evita que o container "estique" além do conteúdo */}
        <div className="bg-white border border-zinc-200 rounded-2xl shadow-sm p-8">
          ...
          <div className="mb-10 border-b border-zinc-100 pb-6">
            ...
            <p className="text-sm text-zinc-500 mt-1">Manage your personal information and public profile</p>
          </div>
          ...
          <div className="flex flex-col md:flex-row gap-12">
            ...
            <div className="flex flex-col items-center md:items-start w-full md:w-1/3">
              ...
              <div className="text-center md:text-left">
                <h2 className="font-semibold text-zinc-900 truncate w-full">{name || authUser.user?.name}</h2>
                <p className="text-xs text-zinc-500 truncate w-full">{authUser.user?.email}</p>
              </div>
            </div>
            ...
            <div className="flex-1 space-y-6">
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full h-11 pl-10 pr-3 rounded-lg border border-zinc-200 text-sm focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 transition-all outline-none"
                  />
                </div>
              </div>
              ...
              <div className="pt-2">
                ...
                >
                  {changePassword ? "✕ Cancel password change" : "→ Change security password"}
                </button>
              </div>

              {changePassword && (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-4 bg-zinc-50 rounded-xl border border-zinc-100 animate-in fade-in duration-300">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full h-10 px-3 rounded-md border border-zinc-200 text-sm focus:border-orange-500 outline-none"
                    />
                  </div>
                  <div>
              ...
              <div className="pt-4 border-t border-zinc-100">
                ...
                <button
                  ...
                  className="w-full md:w-max px-10 h-11 rounded-lg bg-orange-600 text-white text-sm font-bold hover:bg-orange-700 transition shadow-md shadow-orange-200 active:scale-95 disabled:opacity-50"
                >
                  {updateAccountMutation.isPending ? "Saving..." : "Save all changes"}
                </button>
              </div>
            </div>
          </div>
